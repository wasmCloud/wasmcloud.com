...
First we'll build the component from the `/component/` subdirectory:

```shell
wash build
```

Then we'll build the provider from the root of the project directory:
...
wash up
```
And in another terminal, deploy the manifest from the root of the project directory:

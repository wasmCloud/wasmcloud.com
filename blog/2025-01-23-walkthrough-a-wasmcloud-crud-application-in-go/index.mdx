...
Let's take a tour of the major pieces of the project and see how they all fit together.
...
Now we're ready to launch the application:
...
Once the app status is `Deployed`, we can test a POST against our app with `curl`:
...
If you want to iterate more quickly, rather than `wash build` and `wash up` and `wash app deploy`, from your project directory you can simply run:

```shell
wash dev
```

This will start a dev loop that automatically builds and deploys your app and continuously watches for changes. (If you would like to try `wash dev`, I'd recommend first cleaning up your existing deployment according to the instructions below.)

...
At the same time, the scalability and time-savings of fully-managed services can be invaluable. A BYOC model gives enterprises both sovereignty and simplicity, synthesizing the best of DIY and SaaS.
...
[WebAssembly components are portable, interoperable binaries](https://www.wasmcloud.com/docs/concepts/components) that can import and export functionality over language-agnostic interfaces. They're **tiny** (KBs to MBs), **start up in milliseconds (with no cold start)**, and **composable**, meaning that they can be combined like Lego blocks into a single binary. 
...
wasmCloud provides out-of-the-box load balancing, failover, and other features for systems that span across environments such as clouds, edges, on-prem datacenters, and indeed Kubernetes clusters, since [wasmCloud is happy to run standalone or in an org's existing Kubernetes](https://www.notion.so/docs/kubernetes).
...
One team deploys a piece of the platform, and other teams need to adopt it. Imagine an enterprise application called OneQueue that might be used across an organization. With a component-powered application plane like wasmCloud, everyone across the org can adopt OneQueue on a plugin model. You can see how this might be deployed in a single region in the diagram below, with OneQueue (or 1Q in the diagram) deployed across compute planes.

![Single region view of an application running across environments with wasmCloud](./images/byoc.png)

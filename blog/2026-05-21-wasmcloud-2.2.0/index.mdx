...
The implementation is pluggable. Embedders can register a custom TLS provider through `EngineBuilder::with_tls_provider`, which makes it straightforward to swap the default `rustls` backend for a different stack, install a custom root store (corporate CAs, certificate pinning), or front the implementation with an HSM-backed key source. The default TLS provider works out of the box.

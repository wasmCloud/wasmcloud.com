...
image: '/img/how-to-wasi-virt.png'
date: 2024-07-16T09:00:00+05:00
author: 'Eric Gregory'
description: 'How WebAssembly component virtualization works, when you might want to use WASI Virt, and how to get started'
draft: false
---

![How to virtualize WebAssembly components with WASI Virt](/img/how-to-wasi-virt.png)
...
Along with portability and cross-language interoperability, **composability** is a signature feature of components: two or more components can be combined (or "composed") together into one standalone binary, with the constituent components satisfying one another's imports and exports. 

![Animation of components composing together](../../static/img/composed-component.gif)
...
Here's the complete list of WASI interfaces that can be used with WASI Virt:
...
Beyond requirements for running in secure environments, it's useful to remember that virtualization with WASI Virt is a form of build-time composition, and this makes sense for some use-cases more than others, as well as lending itself more and less ideally to certain development workflows. Components that are composed together at build-time are tightly coupled and will generally be updated simultaneously, and will scale together once deployed. If this doesn't make sense for your application, and you don't require an otherwise unavailable core operation system interface, it may make sense to explore dynamically [linking components at runtime](/docs/concepts/linking-components/linking-at-runtime). 

But back to our original example: what if we need WASI Virt to make use of environment variables? Let's take a look at exactly how we can do it.
...
Use `wash inspect` to look at the WIT for this component:
...
Now we can use WASI Virt to compose the `pong_s.wasm` binary into an encapsulating component with an environment variable `PONG` set to `demo`. Our new, composed component will be named `virt.wasm`.
...
* The input component is specified as `build/pong_s.wasm`
...
* The `-e` argument sets an environment variable `PONG` to `demo`
* The output component is named `virt.wasm`

Check out the WIT for the new virtualized component:
...
We get the environment variable value `demo` back in the hello world message. Virtualization achieved!

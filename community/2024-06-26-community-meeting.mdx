...
* Brooks has brought a demo application that shows off wasmCloud's new secret support&mdash;essentially a modified version of the Redis kvcounter example. When performing the usual `curl` against the app, he gets an `unauthorized` response. If he gives a password, he *can* authenticate. This is a wasmCloud application comprised of a component and two providers, one of which is a modified Redis keyvalue provider. 

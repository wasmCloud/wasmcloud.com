...
Brooks moved the **workload identity RFC** onto the roadmap under research. Joonas and Colin have already landed early features — connecting to NATS with workload identity and letting a component fetch its own identity, as demoed at KubeCon — but an open upstream SPIFFE issue may change how it's implemented, especially for sidecarless service meshes like **Istio ambient mode** (the topology wasmCloud's lattice already resembles). The team agreed identity should be optional and largely invisible: "just like Istio, just like Kubernetes — wasmCloud should work with it, but you shouldn't need it."

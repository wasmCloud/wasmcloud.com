...
keywords:
  ...
  - wasi tls
chapters:
  - { seconds: 0, label: "Pre-show chat and agenda setup" }
  - { seconds: 326, label: "Meeting kickoff and agenda overview" }
  - { seconds: 362, label: "Eric's Rust template refactor and handler terminology" }
  - { seconds: 536, label: "Handler naming rationale and call for PR reviewers" }
  - { seconds: 702, label: "Workload config proposal: environment variables and secrets" }
  - { seconds: 1071, label: "Security considerations and guardrails for secrets handling" }
  - { seconds: 1472, label: "Roadmap review: microbenchmarking and automated releases" }
  - { seconds: 1685, label: "Test binary build problem and proposed solutions" }
  - { seconds: 1990, label: "WASI TLS proposal and roadmap addition" }
  - { seconds: 2099, label: "WASI TLS deep dive: scope and Rust crate support" }
  - { seconds: 2521, label: "Host component plugins and native vs WASI plugin architecture" }

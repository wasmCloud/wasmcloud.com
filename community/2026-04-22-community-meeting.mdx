---
date: '2026-04-22'
title: 'Meeting Agenda - 2026-04-22'
---

## Agenda

{/* Update before the start of the meeting with planned agenda items */}

- DEMO: @cdmurph32 wasi-webgpu with p2 model serving
- Train release model discussion [#5071](https://github.com/wasmCloud/wasmCloud/issues/5071)
- Benchmarking @ricochet
- Review Q2 roadmap status @ricochet

{/* truncate */}

## Meeting Notes

{/* Add summary of topics during the meeting. Each Topic should have an H3 heading. A list is usually sufficient for the recap */}

### Topic 1

- Note 1
- Note 2

### Topic 2

- Note 1
- Note 2

## Weekly highlights

### Issue of the Week

- [title](https://github.com/wasmcloud/wasmcloud/issues/)
  - summary

### Documentation of the Week

- [title](/docs/v1/intro)
  - summary

## Community Updates

### Tune in…

{/* Add any upcoming events/streams/etc */}

-

### Catch up…

{/* Add any blog posts/articles that the community might find interesting */}

-

## Recording

{/* Automated - No need to adjust this unless automation fails */}

<YouTube url="https://www.youtube.com/watch?v=18VFhGzObdY" />

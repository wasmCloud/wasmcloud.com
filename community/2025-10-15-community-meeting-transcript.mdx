...
**Yordis Prieto** [01:33](https://youtu.be/3DlUjl8gQVs?t=93)

All right, Brooks, you're the host. Everybody.

**Brooks Townsend** [01:38](https://youtu.be/3DlUjl8gQVs?t=98)

How's it going? Hey, Ian, good to see you again. Don't let me put you too much on the spot — we didn't start yet. I heard maybe you have a demo for today.

**Ian** [01:53](https://youtu.be/3DlUjl8gQVs?t=113)

Yeah, I was just mentioning that. I think we want to actually hold off another week for that, because I wasn't able to get the demo working on wasmCloud yet, mainly due to issues with WASI proposal inconsistencies, since we're trying to target multiple runtimes. But next week for sure — it should have something better by then.

**Brooks Townsend** [02:11](https://youtu.be/3DlUjl8gQVs?t=131)

Cool. Yeah, no, that sounds great. I saw in the thread — was it `wasi:config` or `wasi:runtime-config` that was giving you trouble?

**Ian** [02:23](https://youtu.be/3DlUjl8gQVs?t=143)

It's another thing.

**Brooks Townsend** [02:24](https://youtu.be/3DlUjl8gQVs?t=144)

Well, I think `wash` uses the older one — I always get it mixed up, but it's either it used to be `config-runtime` and now it's `runtime-config`, or the other way around. Inside of `wash`, I think I built in support for the older one, using the previous iteration. So I think I should probably just generate bindings and implement both, because they're the exact same interface. It's just named `config-runtime` or `runtime-config`.

**Ian** [02:57](https://youtu.be/3DlUjl8gQVs?t=177)

That's what I was noticing. I was trying to look at the repo on GitHub and the Wasmtime host implementation, and it's like there's something I'm missing here. So I'm glad to know it's not just me in terms of the issues with it.

**Brooks Townsend** [03:15](https://youtu.be/3DlUjl8gQVs?t=195)

Yeah, no, not at all. Just a little confusing, but it makes sense — this is one of the reasons we are trying to have these interfaces, especially the draft interfaces, be something you can swap in and out, because it kind of stinks to have a bunch of stuff pinned to the old version.

**Ian** [03:39](https://youtu.be/3DlUjl8gQVs?t=219)

It's kind of funny that that isn't the whole purpose of the Wasm interface — it's supposed to be solving that problem, kind of.

**Bailey Hayes** [03:46](https://youtu.be/3DlUjl8gQVs?t=226)

The only standard interfaces are in wasmCloud — `wasi:http`, nothing else is standard. Everything else is depending on some random fork of something, and it's not even enabled by default. If you're running it in wasmCloud, you're opting into using a random draft thing that hasn't been voted on. All those are phase one — anything could be phase one. In the WASI world, basically you had an idea of what phase one is.

**Ian** [04:21](https://youtu.be/3DlUjl8gQVs?t=261)

Got it. Okay, that makes a lot of sense, though.

**Brooks Townsend** [04:24](https://youtu.be/3DlUjl8gQVs?t=264)

I mean, complete devil's advocate — we 100% in `wash` could do both interfaces, we could implement all the drafts and move forward. But it's hard.

One thing, Ian — once we start streaming, would you be interested in doing a general introduction for who you are and what you're working on?

**Ian** [05:02](https://youtu.be/3DlUjl8gQVs?t=302)

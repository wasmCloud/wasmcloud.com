...
- Brooks has been working on a banking application - a microserice written in go for wasmCloud that has a UI and keeps track of transactions over time.
- This application perfectly demonstrates how to bundle authentication, secrets, frontend, Go app, golang OAuth library - compiled into a componemt and running in wasmCloud.
...
- User info is integrated from github with secure githib token - cookie created with user info - which is how you stay logged in.
- UI is super nice - we'll share the example in the repo soon!

Massoud: What happens if I distribute this component to different envrionments? A: you would have the same experience. When you send a request, it is handled by 3 different components - doesn't matter which one as you're served on the individual request. If you have the concept of a 'session' you may need to be careful which component you're targeting.
Tyler: How do I authorise requests in an edge-facing component and pass the user context/auth along? Pre-authorise? Tokenization is done up-front, no need to re-auth but rigorous approach would be to keep around the OAuth token as a cookie - sent to the backend on request throughout the user journey.
...
### Please note: Next week's wasmCloud Wednesday will not be next Wednesday as it's Christmas Day. Instead, next week's meeting will take place at 1pm on Thursday 26th December. 

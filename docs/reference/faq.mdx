...
wasmCloud applications are composed of [components](/docs/concepts/components) and [capability providers](/docs/concepts/providers), which are [linked](/docs/concepts/runtime-linking) together at runtime. Decoupling business logic from capabilities is part of the design philosophy of wasmCloud.
...
There are several kinds of secrets involved in distributed applications. Configuration for components and providers can contain sensitive data. Today, these values are associated with [runtime links](/docs/concepts/runtime-linking), or given to providers on startup The host itself is also configured with sensitive data, which are passed to the host [on startup](/docs/reference/host-config).

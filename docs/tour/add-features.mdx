...
Using the [Query](https://pkg.go.dev/net/url#URL.Query) method on the incoming request URL, we can check the request for a name provided in a query string, and then return a greeting with that name. If there isn't one or the path isn't in the format we expect, we'll default to saying "Hello, World!"
...
func handleRequest(w http.ResponseWriter, r *http.Request) {
	name := r.URL.Query().Get("name") // [!code ++]
	if len(name) == 0 { // [!code ++]
		name = "World" // [!code ++]
	} // [!code ++]
...
func handleRequest(w http.ResponseWriter, r *http.Request) {
	...
	logger.Info(fmt.Sprintf("Greeting %s", name)) // [!code ++]

	kvStore := store.Open("") // [!code ++:11]
	...
	w.Write([]byte(fmt.Sprintf("Hello x%d, %s!\n", value, name)))
	w.Write([]byte(fmt.Sprintf("Hello, %s!\n", name))) // [!code --]
}

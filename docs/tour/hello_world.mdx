...
To start a host, simply run `wash up`. This will start a host with default configurations which is perfect for this tutorial, and you can exit at any time with `CTRL+c`
...
Now that we have a host to run our application on, let's generate a new actor project. This is where our "Hello World" example begins. If you're running your host in the foreground, just switch to another terminal window to run these commands.
...
Congratulations, just ran a WebAssembly application!
...
WebAssembly can be easily scaled due to its small size (this actor clocks in at `201kb`) and it's easy to scale your applications with wasmCloud. Let's scale up our hello world application to 50 replicas by editing `wadm.yaml`:

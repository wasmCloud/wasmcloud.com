...
Now that you've made an update to your component, you can use wash to stop the previous version. You can supply either the name of the component or the full 56 character signing key to the `wash stop component` command. **wadm** will take care of starting the new local copy from the updated file.

```bash
wash stop component http-component
```
...
world hello {
  import wasi:keyvalue/atomic@0.2.0-draft; // [!code ++]
  import wasi:keyvalue/eventual@0.2.0-draft; // [!code ++]
...
func (h HttpServer) Handle(request HttpRequest, responseWriter HttpResponseWriter) {
  ...
  count := http.WasiKeyvalue0_2_0_draft_AtomicIncrement(bucket, name, 1).Unwrap()
...
import { increment } from 'wasi:keyvalue/atomic@0.2.0-draft';
//@ts-expect-error -- these types aren't currently generated by JCO
import { Bucket } from 'wasi:keyvalue/types@0.2.0-draft';

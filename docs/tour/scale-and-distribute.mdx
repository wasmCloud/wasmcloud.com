...
These requirements will ensure that 80% of the replicas are placed in `us-east-1` and 20% in `us-west-1`. You can specify any number of zones and weights to distribute your components across multiple locations. Each requirement matches directly with a [label](/docs/1.0/deployment/hosts/labels) that must be present on a host, and your currently launched host doesn't have any specified labels. Let's launch two more hosts with the required labels, and then deploy our new application.

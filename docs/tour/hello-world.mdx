...
Initially, you'll have to clone the [wasmCloud repository](https://github.com/wasmCloud/wasmCloud). Then, once you have the repository locally, you'll have to `cd` into the directory `examples/docker/`.

There, you'll find many examples to run with `docker compose`.

If you want to install the full wasmCloud host infrastructure (NATS, Graphana, Otel, wasmCloud, Prometheus), simply run `docker compose --file docker-compose-full.yml up`.

For a more minimal infrastructure (NATS, wasmCloud), you can run `docker compose --file docker-compose-minimal.yml`.

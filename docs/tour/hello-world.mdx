...
Looking for an introduction to the basic concepts of WebAssembly components and wasmCloud? Check out [What Is wasmCloud?](/docs/intro) or [Concepts](/docs/category/concepts) first.
...
We'll use `wash` and [wadm](/docs/category/declarative-application-deployment-wadm), the wasmCloud application deployment manager, to start this component and the httpserver provider, and link them together to configure them.

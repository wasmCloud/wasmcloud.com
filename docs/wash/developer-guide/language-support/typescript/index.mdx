...
about: WebAssembly
mentions: [TypeScript, wasmCloud, JCO, ComponentModel]
...
| `jco guest-types` | Generate TypeScript type definitions from WIT interface files |
...
When you run `wash build` on a TypeScript project, `wash` executes the build script defined in your `package.json`, which typically calls `jco guest-types`, `tsc`, and `jco componentize` in sequence.
...
├── generated/
│   └── types/           # Generated by `jco guest-types` — TypeScript definitions from WIT
├── src/
...
1. **`generate:types`** — `jco guest-types` reads your WIT files and generates TypeScript type definitions into `generated/types/`. This gives you IDE autocompletion and type checking for WASI interfaces.
...
These guides walk through adding WASI interfaces to a TypeScript component:
...
The patterns in these guides (declaring WIT imports, importing interfaces against ambient module declarations generated by `jco guest-types`, handling serialization) apply to any WASI interface you add to a TypeScript component.

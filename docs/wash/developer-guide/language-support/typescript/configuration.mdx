...
:::note[Compatibility with jco-std fire()]
The Hono env pattern uses `self.addEventListener('fetch', ...)` directly rather than the `fire()` helper from `@bytecodealliance/jco-std`. If you're using `fire()`, read config with `getEnvironment()` inside each route handler instead.
:::
...
1. **Add `import wasi:cli/environment@0.2.0` to `wit/world.wit`**.

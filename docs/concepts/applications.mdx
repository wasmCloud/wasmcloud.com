...
Applications combine all of the previous concepts into a declarative, deployable unit. wasmCloud supports the imperative management of [components](/docs/concepts/components), [providers](/docs/concepts/providers), and [links](/docs/concepts/runtime-linking), which can be useful for experimentation or debuggging. However, these are generally considered low-level operations. wasmCloud supports complex distributed applications that can be dynamically adjusted at runtime, and managing a combination of components, providers, links, and configuration one command at a time quickly becomes unwieldy.

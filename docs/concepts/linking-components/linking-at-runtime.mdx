...
The wasmCloud host provides several functions available for import by components:

* Logging
* Random number generation
* Clocks

For example, the wasmCloud host's built-in logging function could satisfy a component's logging import. Host functions form wasmCloud's **trusted compute base**&mdash;they represent highly-vetted code that provides basic, commonly-used functionalities. 

To use a host function, you need *only* include the import in a component's `world.wit` and call it using the interface in question. It is not necessary to include host functions in a deployment manifest&mdash;since they are already present on any wasmCloud host, the host will fulfill the imports automatically.
...
export!(HttpServer);
```
Remember: it is not necessary to include host functions in a deployment manifest.
...
<details><summary>Example: Two Components</summary>
...
<details><summary>Example: Unpacking the Quickstart</summary>
...
- For more on Wadm deployment manifests, see the Toolchain section for [Declarative Application Deployment (Wadm)](/docs/category/declarative-application-deployment-wadm).

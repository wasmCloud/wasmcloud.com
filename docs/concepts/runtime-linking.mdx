...
From the [component's](/docs/concepts/components) point of view, code _only_ refers to an interface, e.g. `wasi-keyvalue`. This is because components do not know about (nor should they) _which_ specific provider is on the other side of a link.
...
Runtime links are defined in [wadm manifests](/docs/ecosystem/wadm/). Here is an example of a definition for a link from a component to a provider:

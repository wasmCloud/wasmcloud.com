...
The [actor model](https://en.wikipedia.org/wiki/Actor_model) is a model of concurrent computation that's been around since **1973** and is as meaningful and applicable today than it was 50 years ago. In this model, the **actor** is the primitive building block of concurrent computation. Actors are "black-box" computational entities that can only communicate with other actors by passing messages and creating new actors.
...
wasmCloud actors are **stateless**, allowing hundreds or thousands of copies of an actor to be horizontally scaled across a cluster to meet compute demands. When an actor needs state, it obtains it easily through the use of a capability provider. By using capability providers, wasmCloud can ensure that no matter which instance of an actor is invoked, or where it's running in the cluster, it can still dynamically respond to the state of the larger application.

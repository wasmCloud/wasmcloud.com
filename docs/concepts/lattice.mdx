...
The lattice is a self-forming, self-healing mesh network that provides a unified, flat topology across any number of environments, clouds, browsers, or even hardware. A lattice allows [actors](./actors.mdx), [capability providers](./capabilities.mdx), and [hosts](./hosts.mdx) to communicate with each other across any number of infrastructure barriers. A network in wasmCloud should "just work", which enables short developer feedback loops as well as production performance and resilience.

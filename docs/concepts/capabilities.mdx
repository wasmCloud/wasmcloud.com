...
A **_capability_** is an abstraction over a non-functional requirement, some functionality that is not considered part of the core business logic. Actors depend on capabilities to fulfill their business logic, and these capabilities are provided by — you guessed it — capability providers.
...
The following section was written before WASI Preview 2 was stable. wasmCloud supports the [wasi-cloud-core](https://github.com/WebAssembly/wasi-cloud-core) interfaces, the existing `wasmcloud:*` interfaces, and custom interfaces written by third parties.
...
- be [signed](/docs/hosts/security) with a the appropriate capability contract ID, which happens automatically during the actor build process
- be [linked at runtime](./runtime-linking.mdx) to the capability provider

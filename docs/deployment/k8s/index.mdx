...
Download the operator from GitHub and navigate to the `quickstart` subfolder:

```shell
git clone https://github.com/wasmCloud/wasmcloud-operator.git
```
```shell
...
```
Install the upstream NATS Helm chart to start a cluster with the `values.yaml` file from the quickstart folder:
...
When you `kubectl apply` a [wasmCloud application manifest](../../ecosystem/wadm/model.md), the cluster will automatically provision the component workload with wasmCloud. Below is the `hello-world-application.yaml` manifest included in the quickstart directory:

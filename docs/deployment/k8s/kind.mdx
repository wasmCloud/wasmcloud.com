...
In addition to kind and the **container tooling** a kind cluster requires to run, this guide uses **Helm** and the **wasmCloud Shell (`wash`)** CLI. (For an alternative, script-driven kind quickstart using Kustomize, see the [community contributions](https://github.com/wasmCloud/wasmCloud-contrib) repository.)
...
```
Use `kubectl exec` to shell into the host container:

```shell
kubectl exec -it pod/$WASMCLOUD_HOST_POD -c wasmcloud-host -- bash
```
Install `curl` in the host container:

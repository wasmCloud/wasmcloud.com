...
:::info
When using `wash up` for local development, wash overrides this behavior and enables loading from file references, via the `--allow-file-load` [option](/docs/cli/wasmcloud) on the host. If for some reason it is necessary to start from a file reference in production, this option can be enabled.
:::

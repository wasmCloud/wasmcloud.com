...
This is intended to be bound to monitoring tools (e.g., Prometheus) separately.
...
| `runtime.wasmcloud.dev` | `hosts` | create, delete, get, list, patch, update, watch |
| `runtime.wasmcloud.dev` | `hosts/status` | get, update, patch |
...
- [Workload Security](../workload-security.mdx) — the WebAssembly sandbox model, `allowedHosts`, and Kubernetes NetworkPolicy for securing what workloads can do
- [Secrets and Configuration Management](./secrets-and-configuration.mdx) — RBAC considerations for restricting access to Kubernetes Secrets used by components

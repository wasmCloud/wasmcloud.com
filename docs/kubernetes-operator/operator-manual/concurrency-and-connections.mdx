...
`maxConcurrency` and `poolSize` are set on the component entry; `replicas` is set on the `WorkloadDeployment`. A single component's warm capacity is `poolSize × maxConcurrency` in-flight calls. Multiply by `replicas` for the deployment's total. Each of those calls can open its own outbound connections, so the connections a workload needs grow with the same product. That is the interaction to keep in mind: **instance concurrency sets the demand, connection quotas set the ceiling.**
...
Two separate things are at work here, and only one of them is a knob you set:
...
- **The connection ceiling is the knob.** `maxOutboundHttpConnectionsPerWorkload` (default `128`) caps how many outbound HTTP connections the workload may hold at once, across every destination. This is the limit a request waits behind, and it is what you raise when peak demand outgrows the default.

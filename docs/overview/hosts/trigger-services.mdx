...
A trigger service that exports `wasmcloud:messaging/handler` receives inbound messages through the messaging ingress. As of wasmCloud 2.6.0, the built-in messaging plugins deliver through this ingress for **service** workloads: a service exporting the handler receives every message on its pinned instance. A plain component exporting the same handler runs per message, except that since 2.9.0 a component exporting the async `handler@0.3.0` with `poolSize` set serves deliveries on its warm instance pool, overlapping deliveries up to its `maxConcurrency`.

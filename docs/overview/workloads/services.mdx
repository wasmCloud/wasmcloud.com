...
![services with isolated network namespaces](../../images/multiple-services.webp)

## Communication flow

Services can call [interfaces](../interfaces.mdx) exported by components:

![service to component flow](../../images/service-to-component-flow.webp)
...
![component to service flow](../../images/component-to-service-flow.webp)

## How services use interfaces

Services are not limited to TCP&mdash;they can use your own [custom interfaces](../interfaces.mdx) defined in WIT. The `wasi:sockets` TCP bind permission is simply a special capability that services receive, enabling them to act as TCP listeners within the workload boundary.
...
Services can import any host interface that a [plugin](../hosts/plugins.mdx) provides. The `host_interfaces` field on a workload lists WIT interfaces that need to be resolved by host plugins. When the runtime binds plugins, it checks which WIT interfaces the service's world needs and binds matching plugins&mdash;treating services the same as components in this regard.
...
![connection pooling diagram](../../images/connection-pool.webp)
...
![in-memory diagram](../../images/in-memory.webp)

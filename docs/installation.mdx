...
<details><summary>What's in that install script?</summary>
It is always a good idea to inspect scripts before piping them directly into `sudo bash`. Below are the contents of `script.deb.sh` for your verification.
...
Now that `wash` is installed, let's [get started](/docs/tour/hello_world).

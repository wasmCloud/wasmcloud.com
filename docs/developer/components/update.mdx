...
`wash dev` starts a local wasmCloud host (if you don't already have one started), builds your component,
...
Currently, `wash dev` only will start the component, so you'll have to follow the steps in the [running
the component](./run) section to start the providers your component needs. We hope to automate as much of
...
This will watch your project's files and, whenever there's a change, recompile and update the component in the host. Try iterating through a few changes in the [Adding Capabilities](/docs/tour/adding-capabilities) section to see how it works.

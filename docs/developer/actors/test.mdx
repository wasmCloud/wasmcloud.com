...
Let's revert our actor to the simpler initial version. Our actor is laid out in a way that is difficult to test properly. Let's look at the function signature inside of the `impl` block:
...
```shell
# Override wasm32-wasi with default rustc target
...
Running this test requires you to override the default `wasm32-wasi` target that is used to build actor WebAssembly modules. You can find your default target by running `rustc -vV` and looking for the `host:` entry, or by running `rustup default` and removing `stable-` or `nightly-` from the output.

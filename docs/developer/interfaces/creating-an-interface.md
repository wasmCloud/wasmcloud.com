...
To add this interface to a component, create a `wkg.toml` file in the top level directory of the component and add the following:
...
`wkg.toml` is a common configuration file used in the Wasm ecosystem for package management. In most cases you don't even need it! However, when you're doing local development, it is often useful to have a `wkg.toml` to let the dependency resolver know where to find the interface.
...
You can then fetch dependencies with `wash wit fetch` (or automatically with `wash build`). If you `ls wit/deps` you'll see a directory containing your custom interface.
...
By default this will output a file with the same name as your package with a `.wasm` extension. See the help documentation for `wash wit build` for other options for selecting a different directory or file name. By default, `wash wit build` will not output any dependencies to the `wit/deps` directory. If you need the dependencies available locally (for debugging or generating documentation), run `wash wit fetch` to fetch the dependencies and output them to the `wit/deps` directory.

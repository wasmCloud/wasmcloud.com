...
Invocations are only allowed between components and providers that have been [linked](/docs/concepts/runtime-linking). Omitting a link will result in a runtime error when the host denies the invocation:

...
<Tabs groupId="lang" queryString>
  ...
  By default, the compiled `.wasm` binary for a Rust project is generated at `/target/wasm32-wasip2/release/`. The output path for the compiled `.wasm` binary [can be configured via `wash`'s `config.json` file](../wash/config.mdx).
  </TabItem>
  {/*

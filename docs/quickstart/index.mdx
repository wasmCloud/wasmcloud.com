...
First, we need to install the latest version of `wash` (**`2.0.0`**). If you've already installed `wash`, skip ahead to [Choose your language](#choose-your-language).
...
Verify that `wash` is properly installed and check your version for **`2.0.0`** with:

```bash
wash -V
```

If `wash` is installed correctly, you will see the version string printed, for example `wash 2.0.0`.

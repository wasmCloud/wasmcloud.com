...
function AuthorTitle({ title }: { title: string }) {
  return (
    <small className={clsx(styles.authorTitle, 'avatar__subtitle')} title={title}>
      {title}
    </small>
  );
}
...
function AuthorName({ name, as }: { name: string; as: Props['as'] }) {
  if (!as) {
    return <span className={clsx(styles.authorName, 'avatar__name')}>{name}</span>;
  } else {
    return (
      <Heading as={as} className={clsx(styles.authorName, 'avatar__name')}>
        {name}
      </Heading>
    );
  }

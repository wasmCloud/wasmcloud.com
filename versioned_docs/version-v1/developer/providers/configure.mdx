...
sidebar_position: 3
draft: false
description: "Pass runtime configuration to wasmCloud capability providers using wash config or wadm manifests."
---
...
Capability providers can be passed a map of configuration at runtime to use when executing. This configuration should first be defined using [wash](/docs/v1/ecosystem/wash/) or as a part of your [wadm](/docs/v1/ecosystem/wadm/) manifest, and then given to the capability provider at runtime.
...
If you're using [wadm](/docs/v1/ecosystem/wadm/) to declaratively define applications, you can use the `config` field to define configuration for your provider. Wadm will automatically manage creating the configuration for you and supplying it to the provider at runtime.
...
If you're using [wash](/docs/v1/ecosystem/wash/) to imperatively start your component, you can first define configuration using the `wash config` subcommand, and then supply it alongside your component.
...
We'll use the [wasmCloud HTTP Server settings](https://github.com/wasmCloud/wasmCloud/blob/main/crates/provider-http-server/settings.md) as an example. We'll [link](/docs/v1/concepts/linking-components/) the HTTP server to a component, supplying a default configuration with reasonable values and an additional configuration to change the address for that specific component. To create the default configuration, we can use the `wash config put` command:

...
templating facilities when using `wash new` for creating provider projects quickly.

As most [wasmCloud providers][docs-providers] are written in Rust, there are many examples of Rust providers, along with
projects that are explicitly meant to showcase how to use the provider SDK crate ([`wasmcloud-provider-sdk`][crate-wasmcloud-provider-sdk]):

- [(example) Custom Template Provider][provider-example-custom-template]
- [(example) NATS capability provider][provider-example-nats-messaging]
- [wasmCloud HTTP Client provider][provider-http-client]
- [wasmCloud Rust KV provider][provider-kv-redis]

:::tip
For a full listing of officially supported wasmCloud capabilities, see the [capability catalog][cap-catalog]
:::

For a listing of Rust component tooling, see [**Building components with Rust**](/docs/v1/developer/languages/rust/components).

[crate-wasmcloud-provider-sdk]: https://crates.io/crates/wasmcloud-provider-sdk
[provider-kv-redis]: https://github.com/wasmCloud/wasmCloud/tree/main/crates/provider-keyvalue-redis
[provider-http-client]: https://github.com/wasmCloud/wasmCloud/tree/main/crates/provider-http-client
[provider-example-custom-template]: https://github.com/wasmCloud/wasmCloud/tree/main/examples/rust/providers/custom-template
[provider-example-nats-messaging]: https://github.com/wasmCloud/wasmCloud/tree/main/examples/rust/providers/messaging-nats
[cap-catalog]: /docs/v1/capabilities
[docs-providers]: https://wasmcloud.com/docs/v1/concepts/providers/

## Get started

### Prerequisites

To build a capability provider in Rust, you'll need...

* [cargo](https://go.dev/doc/install) 1.23.0+
...
wasmCLoud providers by default use [NATS][nats] to communicate, so testing a provider often means sending messages on NATS topics
in orer to trigger actions in the provider.

[wash]: /docs/v1/ecosystem/wash
[cargo]: https://doc.rust-lang.org/cargo
[nats]: https://nats.io/

### Create a new provider

Use `wash new` to template out a new provider project:

```console
wash new provider custom --template-name custom-template-rust
```

You can then cd into the new folder:

```console
cd custom
```

In the new project directory we can immediately build our provider:

```console
wash build
```

### Read & Modify the generated code

Modern WebAssembly and wasmCloud are *interface-driven* -- so the first file that is likely worth looking at is the WIT interface
for the generated provider:
...
While regular Rust is not natively WIT-aware like wasmCloud components are, code generated by `wash new` adds libraries for dealing
with WIT interfaces as a wasmCloud provider.

The Rust code generated by the template for a wasmCloud Rust provider is well commented, describing the required functionality for
wasmCloud providers (keep in mind the WIT interfaces this provider is expected to fulfil):
...
Functionality for communicating with wasmCloud hosts and fulfilling the requirements for wasmCloud providers is managed by
the [`wasmcloud-provider-sdk`][crate-wasmcloud-provider-sdk] crate.

As wasmCloud providers are binaries, there is also a minimal `src/main.rs` which includes the entrypoint for the provider:
...
:::tip
[wasmCloud providers][docs-providers] are launched and managed by [wasmCLoud hosts][docs-host], and communicate via NATS.
:::

This code implements two functions defined in our WIT world: `process-data` and `system-info` (the Rust bindings translate them
to the more idiomatic `process_data` and `system_info` respectively).

The basic structure of the provider is as follows:

* Interface definitions in a WIT world
* Provider logic implementation in `provider.rs`
* Basic binary executable scaffolding in `main.rs`

[docs-host]: https://wasmcloud.com/docs/v1/concepts/hosts/

### Build the provider

To build the provider, you can run:
...
Built artifact can be found at "/tmp/custom/build/wasmcloud-provider-custom-template.par.gz"
```

`wash build` produced a `.par.gz` file for the provider, and placed that in the `build` directory. Providers can be distributed
as `.par.gz` files, but are more commonly distributed as OCI images (see [`wash push`][docs-cli-wash-push]).

[docs-cli-wash-push]: /docs/v1/cli/wash#wash-push

### Run on wasmCloud

To run our provider on wasmCloud first we launch a wasmCloud instance with `wash`:

```console
wash up
```

This command won't return, but in another terminal, we can deploy the provider we built with `wash`:

```console
wash start provider build/wasmcloud-provider-custom-template.par.gz custom
```

We can confirm our provider is running by running `wash get inventory`:
...
```
  ...
  ND73K3C67KFMWWVKJ7IT3QM4YSAX5UNC6NVYDEPQMBTGTVEHJGQ37RQV      icy-sun-9218
...
With a component running on wasmCloud, we can now link components and other providers to it,
and write components that make use of custom interfaces and functionality.

New components that use your provider must `import` WIT interfaces that are that is `export`ed by your provider,
but other than that caveat they are identical to the [existing Rust example components][rust-examples].

[rust-examples]: https://github.com/wasmCloud/wasmCloud/tree/main/examples

## Next steps

* Explore [capabilities](/docs/v1/capabilities/) you can use in your wasmCloud application.
* Learn [how to build a component in Rust](/docs/v1/developer/languages/rust/components).

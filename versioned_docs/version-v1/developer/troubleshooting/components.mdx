...
Before you follow this section, consider [configuring the log level](/docs/v1/developer/troubleshooting/host#changing-the-log-level) to `debug` for more information when troubleshooting.
...
To fix this error, you need to set the `--allowed-insecure` [argument on the host](/docs/v1/cli/wasmcloud).
...
Invocations are only allowed between components and providers that have been [linked](/docs/v1/concepts/linking-components/). Omitting a link will result in a runtime error when the host denies the invocation:

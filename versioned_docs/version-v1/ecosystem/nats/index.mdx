...
Streams and buckets play an important role in the scheduling process of the wasmCloud Application Deployment Manager (wadm). See the [wadm](/docs/v1/ecosystem/wadm/) section for more information.
...
For more information on troubleshooting and NATS, see the [troubleshooting](/docs/v1/developer/troubleshooting/host#clear-nats-streams-and-buckets) section.

## Keep reading

- See the [NATS Reference](/docs/v1/category/nats/) for complete reference documentation on NATS usage in wasmCloud
- See the Operator Guide for instructions on [provisioning NATS for wasmCloud](/docs/v1/category/provisioning-nats) in production environments.
- For information on authenticating to NATS with accounts and users, see [Connecting NATS to Wadm](/docs/v1/deployment/wadm/nats-credentials).
- You can [use the `wash` CLI to open a WebSocket port](/docs/v1/cli/wash#wash-up) utilizing [WebSocket support in NATS](https://docs.nats.io/running-a-nats-service/configuration/websocket).

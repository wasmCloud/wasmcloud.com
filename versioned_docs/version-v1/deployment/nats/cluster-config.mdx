...
This configures a single NATS server with JetStream enabled and data stored in a known location. This configuration sets a JetStream domain, which enables fanning out the deployment using [leaf node sidecars](/docs/v1/deployment/nats/leaf-nodes) later.

...
wasmCloud hosts can be configured at start time with a variety of [host config](/docs/v1/reference/host-config) options. However, in some production deploys, it is useful to provide supplemental configuration to a host from an external source.
...
When making a request for configuration, the host provides its [labels](/docs/v1/deployment/hosts/labels) as the payload:
...
In response, the host receives supplemental configuration. At this time, the only configuration returned is a set of credentials for [OCI registries](/docs/v1/deployment/netconf/registries):

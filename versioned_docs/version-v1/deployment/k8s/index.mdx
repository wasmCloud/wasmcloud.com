...
For high-level discussion of our approach to Kubernetes and compatibility with common cloud native tooling, see [wasmCloud on Kubernetes](/docs/v1/kubernetes/).
...
* [wasmCloud host(s)](/docs/v1/concepts/hosts) - Runtime environment for WebAssembly components
* [NATS with Jetstream](/docs/v1/ecosystem/nats/) - Connective technology underlying the wasmCloud [lattice](/docs/v1/concepts/lattice)
* [wasmCloud Application Deployment Manager (`wadm`)](/docs/v1/ecosystem/wadm/) - Scheduler for application lifecycle management
...
2. Interoperability with the [wasmCloud Application Deployment Manager (wadm)](/docs/v1/ecosystem/wadm/) for deploying wasmcloud applications defined in the [OAM spec](https://oam.dev/) format, so that a wasmCloud application can be deployed with a single `kubectl apply`
...
When you `kubectl apply` a [wasmCloud application manifest](/docs/v1/ecosystem/wadm/model), the cluster automatically provisions the component workload with wasmCloud. 

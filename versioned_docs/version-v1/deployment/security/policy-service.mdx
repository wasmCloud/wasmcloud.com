...
:::warning[Planned changes to policy service]
The [**wasmCloud Q3 2025 Roadmap**](https://github.com/orgs/wasmCloud/projects/7) sets out plans for changes to the policy service implementation in the next major release of wasmCloud. The new approach decouples the policy service from the runtime layer and instead integrates with ecosystem-standard tooling. For more information, see the [Roadmap](https://github.com/orgs/wasmCloud/projects/7) and [Issue #4641: “Reduce host responsibility and API surface”](https://github.com/wasmCloud/wasmCloud/issues/4641)
:::

wasmCloud hosts will always enforce a certain level of security by default that _cannot be loosened_. For example, hosts will always validate [runtime links](/docs/v1/developer/troubleshooting/components#missing-runtime-links). Additionally, the wasmCloud **policy service API** can be used to extend and customize policy evaluation, such as by restricting untrusted application components and providers on particular hosts.
...
Policy requests are made on a NATS subject, which is specified by the `--policy-topic` [host config](/docs/v1/cli/wasmcloud) option. (The argument uses the general messaging term "topic" rather than the NATS-specific "subject," but you will simply specify a [NATS subject](/docs/v1/ecosystem/nats/#essential-nats-concepts) here.)

...
It is also possible to enable and overwrite endpoints for only specific signals&mdash;see the pages for [traces](/docs/v1/deployment/observability/tracing), [logs](/docs/v1/deployment/observability/logs), and [metrics](/docs/v1/deployment/observability/metrics) for more details.

While these options are typically passed as part of [host configuration](/docs/v1/cli/wasmcloud), you can launch a wasmCloud observability ecosystem locally by passing environment variables to `wash up`:
...
Signal-specific options on the wasmCloud host are detailed on the [traces](/docs/v1/deployment/observability/tracing), [logs](/docs/v1/deployment/observability/logs), and [metrics](/docs/v1/deployment/observability/metrics) pages. For more information on OpenTelemetry, refer to the [project documentation](https://opentelemetry.io/docs/).

...
description: "Extend your wasmCloud app with custom capabilities, alternative providers, and deployment manifests."
---

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

In our previous tutorials, we started a developer loop and added two [**interfaces**](/docs/v1/concepts/interfaces) (`wasi:logging` and `wasi:keyvalue`) to our application. The `wash dev` process satisfied our application's capability requirements automatically, so we could move quickly and focus on code.
...
:::info[Prerequisites]
This tutorial assumes you're following directly from the previous steps. Make sure to complete [**Quickstart**](/docs/v1/tour/hello-world) and [**Add Features**](/docs/v1/tour/add-features) first.
:::
...
If we want to learn about a capability, we can browse the [**Capability Catalog**](/docs/v1/capabilities/).

Each capability in the catalog consists of an [interface](/docs/v1/concepts/interfaces) and one or more [capability providers](/docs/v1/concepts/providers).
...
- [The wasmCloud built-in logger](/docs/v1/concepts/providers#built-in-providers)
...
- For more information on defining application manifests, see our documentation on the [wasmCloud application deployment manager](/docs/v1/ecosystem/wadm/).
- For more information on capabilities, such as how to create new interfaces and providers, see the Developer Guide pages for [providers](/docs/v1/developer/providers/) and [interfaces](/docs/v1/developer/interfaces/creating-an-interface).

You can continue to build on this application by adding more features, or you can explore additional first-party providers in the [**Capability Catalog**](/docs/v1/capabilities/) to get an idea of what is possible with wasmCloud.

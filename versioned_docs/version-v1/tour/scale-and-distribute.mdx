...
In the previous steps, we started a developer loop, added two [**interfaces**](/docs/v1/concepts/interfaces) (`wasi:logging` and `wasi:keyvalue`), and extended our application with pluggable [**capabilities**](/docs/v1/concepts/capabilities).
...
:::info[Prerequisites]
This tutorial assumes you're following directly from the previous steps. Make sure to complete [**Quickstart**](/docs/v1/tour/hello-world), [**Add Features**](/docs/v1/tour/add-features), and [**Customize and Extend**](/docs/v1/tour/customize-and-extend/) first.
:::

### Declaratively deploying the application

We will continue to use the Redis key-value capability provider as in the [Customize and Extend](/docs/v1/tour/customize-and-extend/) step, but this time we will specify the provider using a declarative [**application manifest**](/docs/v1/ecosystem/wadm/model) written in YAML, much like a Kubernetes manifest.

This `.yaml` file defines the relationships between the components and providers that make up our application, along with other important configuration details. It is conventionally named `wadm.yaml` after the [**wasmCloud Application Deployment Manager**](/docs/v1/ecosystem/wadm/) that handles scheduling.
...
Following the specification in the manifest, wasmCloud instructs the [host](/docs/v1/concepts/hosts)'s WebAssembly runtime ([wasmtime](/docs/v1/ecosystem/wasmtime)) to instantiate **only a single instance of our component** at any time to process incoming requests. As a result, requests received at the same time are processed sequentially, one after the other. 
...
These requirements will ensure that 80% of the replicas are placed in `us-east-1` and 20% in `us-west-1`. You can specify any number of zones and weights to distribute your components across multiple locations. Each requirement matches directly with a [label](/docs/v1/deployment/hosts/labels) that must be present on a host, and your currently launched host doesn't have any specified labels. Let's launch two more hosts with the required labels, and then deploy our new application.

...
The lattice is a self-forming, self-healing mesh network that provides a unified, flat topology across any number of environments, clouds, browsers, or even hardware. A lattice allows [components](/docs/v1/concepts/components), [providers](/docs/v1/concepts/providers), and [hosts](/docs/v1/concepts/hosts) to communicate with each other across any number of infrastructure barriers. A network in wasmCloud should "just work," which enables short developer feedback loops as well as production performance and resilience.

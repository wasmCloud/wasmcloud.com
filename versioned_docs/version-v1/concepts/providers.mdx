...
**Providers** are swappable [host](/docs/v1/concepts/hosts) plugins&mdash;executables (usually dedicated to longer-lived processes) that deliver common functionalities called **capabilities**. Providers are typically responsible for [capabilities](/docs/v1/concepts/capabilities) that are not considered part of the core business logic of an application, such as...
...
You can find a complete list of first-party providers on the [wasmCloud GitHub Packages page](https://github.com/orgs/wasmCloud/packages?repo_name=wasmCloud). First-party providers are named to specify the [interface](/docs/v1/concepts/interfaces) being used and the specific functionality implemented with the interface (e.g. "keyvalue" for the WASI interface and "Redis" for the specific functionality).

First-party providers may be **external** or **built-in**. External providers run on the wasmCloud [lattice](/docs/v1/concepts/lattice/) like components&mdash;they are "external" in the sense that they are external to any given wasmCloud host. All third-party providers are external, and generally, when we talk about providers, we are talking about external providers. There is, however, a small set of **built-in** providers that represent wasmCloud's trusted compute base and are built directly into the wasmCloud host.
...
In [Wadm manifests](/docs/v1/ecosystem/wadm/), you can refer to the [wasmCloud GitHub Packages registry](https://github.com/orgs/wasmCloud/packages?repo_name=wasmCloud) for OCI images of providers&mdash;for example: `ghcr.io/wasmcloud/keyvalue-redis:0.27.0`
...
Built-in providers form wasmCloud's **trusted compute base**&mdash;carefully vetted code providing core functionalities. You can learn more about using built-in providers on the [Linking at runtime](/docs/v1/concepts/linking-components/linking-at-runtime#built-in-providers) page.

## Keep reading

[Continue to learn more about interfaces](/docs/v1/concepts/interfaces) that providers use to communicate with components (or one another), or...

- To learn how to build your own provider, explore the [Provider section of the Developer Guide](/docs/v1/developer/providers/).

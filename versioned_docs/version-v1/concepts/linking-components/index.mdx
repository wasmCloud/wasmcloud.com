...
A [component](/docs/v1/concepts/components/) includes **imports** and **exports** that define what the component depends on and what it can do.
...
* [**Built-in providers**](/docs/v1/concepts/linking-components/linking-at-runtime#built-in-providers) (Runtime): The wasmCloud host provides several functions available for import by components (for example, logging) through built-in providers. 
* [**Other components**](/docs/v1/concepts/linking-components/linking-at-runtime#other-components) (Runtime or build): Components may be composed or linked with wRPC, and runtime links may be made with components running on the same wasmCloud host or another host on the lattice. 
* [**External providers**](/docs/v1/concepts/linking-components/linking-at-runtime#providers) (Runtime): [Providers](/docs/v1/concepts/providers) are stateful host plugins which may facilitate connections to external resources. A component may be linked to an external provider running either on the same wasmCloud host or another host on the lattice via wRPC. (You can learn more about the difference between external and built-in providers on the [Provider](/docs/v1/concepts/providers) page.)

[Linking at runtime](/docs/v1/concepts/linking-components/linking-at-runtime) provides examples of linking with each of these entities.
...
[Continue to learn more about **applications**](/docs/v1/concepts/applications), the abstraction that combines all the pieces you've explored so far, or...

- To learn more about runtime linking and explore examples, see [Linking at runtime](/docs/v1/concepts/linking-components/linking-at-runtime).

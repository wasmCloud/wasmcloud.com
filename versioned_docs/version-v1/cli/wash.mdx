...
* `--nats-version <NATS_VERSION>` — NATS server version to download, e.g. `v2.10.26`. See https://github.com/nats-io/nats-server/releases>/ for releases

  Default value: `v2.10.26`
...
* `--wasmcloud-version <WASMCLOUD_VERSION>` — wasmcloud host version to download, e.g. `v1.4.2`.

   defaults to the [`WASMCLOUD_HOST_VERSION`] if not provided or the latest patch version after that when `wash up` issued, see https://github.com/wasmCloud/wasmCloud/releases> for releases
...
* `--wadm-version <WADM_VERSION>` — wadm version to download, e.g. `v0.18.0`.

   defaults to the [`WADM_VERSION`] if not provided or the latest patch version after that when `wash up` issued, see https://github.com/wasmCloud/wadm/releases> for releases
...
* `-d`, `--work-dir <code-dir>` — Path to code directory
* `-i`, `--ignore-dir <ignore-dir>` — Directories to ignore when watching for changes. This should be set to directories where generated files are placed, such as `target/` or `dist/`. Can be specified multiple times
...
* `--all` — Delete all links present in the cluster (with prompt)

  Default value: `false`
* `--force` — Force an operation that is otherwise seen as risky
...
* `--wit-directory <WIT_DIR>` — Location of project directory containing WIT
...
* `deps` — Fetch dependencies for a component

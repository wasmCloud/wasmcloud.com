...
// [!code ++]
 ...
 // [!code ++]
         ...
         // [!code ++]
            ...
            wasi::http::types::Method::Delete => { // [!code ++]
                // Delete key // [!code ++]
                let _ = wasi::keyvalue::eventual::delete(&bucket, &name); // [!code ++]
// [!code ++]
                // Send HTTP response // [!code ++]
                response_body // [!code ++]
                    .write() // [!code ++]
                    .unwrap() // [!code ++]
                    .blocking_write_and_flush(format!("It's like {name} was never there").as_bytes()) // [!code ++]
                    .unwrap(); // [!code ++]
                OutgoingBody::finish(response_body, None).expect("failed to finish response body"); // [!code ++]
                ResponseOutparam::set(response_out, Ok(response)); // [!code ++]
// [!code ++]

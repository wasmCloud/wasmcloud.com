...
This configures a single NATS server with JetStream enabled and data stored in a known location. This configuration sets a JetStream domain, which enables fanning out the deployment using [leaf node sidecars](/docs/0.82/deployment/nats/leaf-nodes) later.
...
Once you have a running cluster, create the [lattice metadata](/docs/0.82/deployment/lattice/metadata#production-configuration) and [wadm Key Value buckets](/docs/0.82/deployment/wadm/configuration) configured with replication across more than one NATS server. This makes the wasmCloud cluster resilient to node and instance failures.

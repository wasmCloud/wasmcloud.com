...
There are three primary deployments that comprise wasmCloud in a Kubernetes system:

1. **Runtime Operator** — watches for workload CRDs and schedules Wasm workloads onto hosts
2. **Runtime Gateway** — routes incoming HTTP traffic to deployed Wasm workloads
3. **Host Group** — a pool of pods running the wasmCloud `wash` runtime that execute Wasm components
...
The Runtime Operator is the control-plane component that watches for wasmCloud [CRDs](../crds.mdx) and reconciles the desired state. Its responsibilities include:
...
- **Leader election**: When running with multiple replicas, uses a `coordination.k8s.io/v1` Lease in the `default` namespace to elect a single active instance and avoid split-brain reconciliation.
...
The Host Group is a `Deployment` of pods, each running the wasmCloud `wash` binary with the `wash host` command. A Host Group provides the sandboxed execution environment for WebAssembly components. Key characteristics:
...
1. The **Runtime Operator** detects the new CRD, selects a matching host from the Host Group, and schedules the Wasm component onto it.
2. The operator updates the `WorkloadDeployment` status to reflect the scheduling outcome.
3. The **Runtime Gateway** observes the new `Workload` resource and adds a routing rule for HTTP traffic.
4. Incoming HTTP requests hit the Gateway's Service, which forwards them to the appropriate host pod, where the **wash runtime** executes the Wasm component.

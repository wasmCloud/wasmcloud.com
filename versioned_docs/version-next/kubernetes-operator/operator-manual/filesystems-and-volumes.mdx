...
In a wasmCloud deployment, filesystem data is made available to the containerized wasmClouod host, which may in turn pass data to a component.
...
The sample host configuration excerpt below makes a volume called `wasmcloud-data` available to a host with the `public-ingress` label.

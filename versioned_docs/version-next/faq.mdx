...
Instead, wasmCloud v2 uses the concept of a [Workload](./overview/workloads.mdx), which may include one or more components that communicate over interfaces. Components in the same workload are placed on the same host and linked at runtime.

See the [Platform Overview section for more information on the Workload](./overview/workloads.mdx).

...
![wasmCloud v1 architecture](./images/v1-architecture.svg)
...
In v2, the Kubernetes operator (`runtime-operator`) replaces `wadm` as the workload scheduler. State is stored in Kubernetes `etcd`, workloads are defined using Kubernetes CRDs, and capability providers are replaced by host plugins and services.

![wasmCloud v2 architecture](./images/v2-architecture-migration.svg)
...
![v1 to v2 concept migration map](./images/concept-mapping.svg)
...
![Capability provider migration paths](./images/capability-migration.svg)
...
![v2 security and permissions model](./images/security-model.svg)
...
![wasmCloud CRD hierarchy](./images/crd-hierarchy.svg)
...
![v2 workload scheduling flow](./images/scheduling-flow.svg)

1. **Developer compiles** a `.wasm` component from source using `wash build`
...
For existing provider code that you want to migrate with minimal changes, you can **containerize the provider** and run it outside wasmCloud. This requires manually facilitating communication between your components and the provider.

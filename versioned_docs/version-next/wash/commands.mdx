...
:::note[]
When you're creating a new project with `wash new`, you're necessarily outside of a project directory. Use `wash --help` and check the `-C` option to find where new projects will be created by default on your system. To create a new project in your current directory: `wash -C ./ new [OPTIONS] <GIT>`. For example:

```shell
wash -C ./ new https://github.com/wasmCloud/wash.git --name hello --subfolder examples/http-hello-world
```
:::

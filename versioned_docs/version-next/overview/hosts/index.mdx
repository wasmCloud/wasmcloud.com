...
Components cannot directly access capabilities. Instead, the host subscribes to incoming messages and makes outbound requests on behalf of components, using NATS or another transport mechanism. This allows the host to run untrusted components without compromising security.

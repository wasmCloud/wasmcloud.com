...
We can use the [`wasi:keyvalue`](https://github.com/WebAssembly/wasi-keyvalue) interface for interacting with a key value store. Interfaces are declared in [WIT](../overview/interfaces.mdx) (WebAssembly Interface Types) files—adding an import to our component's WIT world grants it access to the capability.

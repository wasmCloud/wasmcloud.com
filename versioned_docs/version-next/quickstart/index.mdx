...
```shell
wash new https://github.com/wasmCloud/wasmCloud.git --name hello --subfolder examples/rust/components/http-hello-world
```
...
* In the subfolder `examples/rust/components/http-hello-world`
...
To start a developer loop or build a component binary from a Go project, `wash` requires the project's WIT world to be specified in a [configuration file](../wash/config.mdx) called `config.json` at `/.wash`. (When you try to build without a config file, `wash` will prompt you, but we'll go ahead and get it out of the way.)

Create the config file, specifying your WIT world as `hello`:

```shell
mkdir .wash && echo '{"build":{"tinygo":{"target":"wasip2","build_flags":[],"disable_go_generate":false,"scheduler":"asyncify","gc":"conservative","opt":"z","panic":"print","tags":[],"no_debug":true,"wit_world":"hello"},"component_path":"build/output.wasm"},"wit":{"registries":[],"skip_fetch":true,"wit_dir":null,"sources":{}}}' > ./.wash/config.json
```

...
For our last step, let's run a more complex demo application on wasmCloud. 

We'll use `kubectl` to deploy **Blobby**, a file manager that enables users to upload, download, and delete files from an HTTP server via a web interface.

```shell
kubectl apply -f TK
```

Once Blobby is deployed, we can open the web UI at TK address.

TK image

- Enumerate what you can do with it

Blobby brings together WASI interfaces like HTTP and Blobstore to create a fleshed-out application that leaves common, reusable functionality (like HTTP and blob storage) to the host, while sandboxing the code in a secure environment where capabilities must be granted explicitly. 

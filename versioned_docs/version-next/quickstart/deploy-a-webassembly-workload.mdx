...
Apply the following manifest to deploy the [`blobby-ui`](https://github.com/wasmCloud/wasmCloud/tree/main/examples/blobby) component from a wasmCloud-hosted OCI image:
...
spec:
  ...
  template:
    spec:
      ...
      components:
        - name: blobby
          image: ghcr.io/wasmcloud/components/blobby-ui:0.5.0
EOF
```

The `host: blobby.localhost.direct` config tells the Runtime Gateway to route requests with that hostname to this component. `blobby.localhost.direct` is a public DNS alias for `127.0.0.1`, so no `/etc/hosts` changes are needed.
...
Once running, try the file server:
